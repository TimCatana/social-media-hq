...
async function getPinterestCompetitorPosts(competitor) {
  const config = await loadConfig();
  const accessToken = await getPinterestToken(config);
  const csvPath = path.join(__dirname, '..', '..', 'bin', 'csv', 'pinterest_competitor_analysis.csv');

  try {
    // Fetch competitor pins
    const pinsResponse = await axios.get(
      `https://api.pinterest.com/v5/users/${competitor}/pins`,
      {
    ...
    const { sortBy } = await prompts({
      type: 'select',
      name: 'sortBy',
      message: 'Sort Pinterest competitor pins by:',
      choices: [
    ...
    console.log(`Pinterest competitor pins sorted by ${sortBy} saved to ${csvPath}`);
    await saveConfig(config);
  } catch (error) {
    console.error(`Pinterest competitor analysis failed: ${error.response ? JSON.stringify(error.response.data) : error.message}`);
  }
}

module.exports = { getPinterestCompetitorPosts };